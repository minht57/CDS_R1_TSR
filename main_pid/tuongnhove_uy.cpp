...
// MULTITHREADS ===========================================================================================//
	...
	//thread_3 = std::thread(tsr_d,std::ref(colorImg),std::ref(tsr_stt),par.S_val,par.S_max,par.V_val,par.V_max);
	// thread_0.join();
    thread_1.join();
    // thread_2.join();
    thread_3.join();
...
// SET STEERING & THROTTLE ================================================================================//
	//theta = (-90.00);
	//api_set_STEERING_control(pca9685,theta);
	api_set_STEERING_control(pca9685,delta_x);
...
// IS SHOW FILE ===========================================================================================//
	...
	char buf[100];
	sprintf(buf, "FRAME ID: %d - delta_x %.1f theta %.1f lanepos %d angle_lane %.1f",frame_id, delta_x, theta, lane_pos, angle_lane);
...
{	
	...
	if(!colorImg_clone.empty())
    {
		...
		{	
            pointMidLane = new_lane_process(grayImage.size(), grayImage, lane, FilteredMidlanePoint.x, par.yCalc, par.point_offset, tsr_stt, lane_pos, angle_lane);
...
void tsr_depth_process(cv::Mat &colorImg, cv::Mat &depthImg, int &tsr__)
{
	...
	else 
	{
		...
		if((!colorImg_clone.empty()) && (!depthImg_clone.empty()))
		{
			...
			Rect boxes;
			detection.objectLabeling(boxes, TSRDepthImg, l_th, h_th, 1000, 4000, 36, 200, 1.2);
