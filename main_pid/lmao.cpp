...
// CAL STEERING ===========================================================================================//
	...
	u_control = 1*delta_x + 0.4*(delta_x + pre_delta_x);  // PI Controller
	pre_delta_x = delta_x;

	if((u_control > 70) || (u_control < -70))
	{
...
// SET STEERING & THROTTLE ================================================================================//
	...
	{
		u_control = -40 + (u_control + 20); //(90 - 40)/(90 - 20) = 0.75
		// if (u_control < -75)
		// 	u_control = -90;
	}
...
// IS SHOW FILE ===========================================================================================//
	...
	char buf[100];
	sprintf(buf, "FRAME ID: %d - delta_x %.1f u_control %.1f theta %.1f lanepos %d ",frame_id, delta_x, u_control, theta, lane_pos);
	...
	if(!colorImg_clone.empty())
    {
		...
		{	
			...
			// {
				...
				pointMidLane = new_lane_process(im_raw.size(), im_raw, lane, FilteredMidlanePoint.x, par.yCalc, par.point_offset, tsr_stt, lane_pos, angle_lane, 0);
			// }
