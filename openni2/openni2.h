...
public:
    ...
    bool getDisparityMapFloat(cv::Mat &disparity);
    bool getBGRImage(cv::Mat &img);
    bool getGrayImage(cv::Mat &gray);
    bool getIRImage(cv::Mat &ir);
    bool getImage(cv::Mat &img, int type);
    void getData(cv::Mat &color, cv::Mat &depth, cv::Mat &gray, cv::Mat &disparity);
    void getData_new(cv::Mat &color, cv::Mat &depth);
    void release();
private:
