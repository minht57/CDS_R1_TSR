...
/// Init video writer and log files ///   
    ...
    string gray_filename = "gray_uy.avi";
	string color_filename = "color_uy.avi";
	string depth_filename = "depth_uy.avi";
...
vector<vector<Point> > regs;
    ...
    { 
       ...
       unsigned int bt_status = 0;
		...
		vector<Rect> boxes;
		vector<int> labels;
		bool test = SignDetection::Instance()->objectLabeling(boxes, labels, depthImg, colorImg, l_th, h_th, 1000, 8000, 50, 200, 1.5);
		if(boxes.size()<=0)
			cout<<"hello"<<endl;
		...
		vector<Point> pointList;
			...
			yTam = yTam + 240 * 3 / 4;
			circle(grayImage, Point(xTam, yTam), 2, Scalar(0, 0, 0), 3);
