...
//cv::imshow("c_depth", depthImg/30);
		...
		cv::Mat result;
		cv::Mat pyrDown;
		detection.objectLabeling(boxes, labels, depthImg, colorImg, result, l_th, h_th, 1000, 8000, 50, 200, 1.5);
		/********************************************************************************************************************/
		if(result.empty())
		cout<<"No object"<<endl;	
		else
			...
			//Mat im_raw = result.clone();
		{
			...
			imshow("im_raw",result);
		}
			vector<int> compression_params;
   			compression_params.push_back(CV_IMWRITE_PNG_COMPRESSION);
    		compression_params.push_back(9);
			imwrite(std::to_string(img_count)+".png", result, compression_params);
		...
		for (int x = 0; x != signLabels.size(); ++x)
		{
     	cout<<"Bien bao so: "<<names[x]<<" .....end"<<endl;
		cout<<"Bien bao so: "<<names.at(x)<<" .....end"<<endl;
		}
		
        cout<<"here"<<names.size()<<"enddd"<<endl;
