...
public:
    ...
    void objectLabeling(std::vector<cv::Rect> &boxes, std::vector<int> &labels, cv::Mat &depth, cv::Mat &color, cv::Mat &object, ushort &low_th, ushort &high_th, int min_pts, int max_pts, ushort min_w, ushort max_w, double ratio);
    void objectLabeling(cv::Rect &rect, cv::Mat &depth, ushort &low_th, ushort &high_th, int min_pts , int max_pts , ushort min_w, ushort max_w , double ratio );
