...
{  
    try
    {
        ...
        parser.add_option("ignore", "Ignore image with error ratio or smaller than target size");
        ...
        if (parser.option("t") || parser.option("cross-validate"))
        {
            ...
            if (parser.option("ignore"))
            {
            	cout <<"List of image ignored:	(not matches ratio or smaller than target size)" << endl;
            	int ci = 0;
		        if (contains_any_boxes(removed))
		        {
		            image_dataset_metadata::dataset data;
		        ...
		        cout << endl;
		        cout <<"Number of image ignored: " << ci << endl;
		    }
            return EXIT_SUCCESS;
        }
