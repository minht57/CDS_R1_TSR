...
SignRecognition::SignRecognize::SignRecognize() {
	...
	signProc.loadDict("/home/ubuntu/DriverlessCarChallenge/jetson/carControl/src/0.3/ObjectRecognition/Dict.txt");

	/// 2. Đặt giá trị ompError cho xử lý.
	signProc.setOmpError(OMP_ERROR);

	/// 3. Load vector v của các biển báo trong thư viện.
	readVDictVector("/home/ubuntu/DriverlessCarChallenge/jetson/carControl/src/0.3/ObjectRecognition/VectorVDict.txt", vDictVector);

	/// 4. Load vector l dùng cho xử lý biển.
	readLVector("/home/ubuntu/DriverlessCarChallenge/jetson/carControl/src/0.3/ObjectRecognition/VectorL.txt", lVector);
	return ;
}
...
int SignRecognition::SignRecognize::recognizeSign(cv::Mat &sign) {
	...
	std::cout<<"num img"<<numImg<<std::endl;
	int ret = -1;
